...
def read_1D_vel_model_to_2D_model_for_fmm(
    oneD_vel_model_z_df, extent_x_m=3000, dxz=[1.0, 1.0]
):
    ...
    vel_model_x_labels = np.arange(0.0, extent_x_m + dx, dx)
    vel_model_z_labels = oneD_vel_model_z_df["depth"].values
    # Create the index grids:
...
def create_2D_LUT(
    oneD_vel_model_z_df, array_centre_xz, extent_x_m=3000.0, dxz=[1.0, 1.0], n_threads=1
):
    ...
    dx = dxz[0]  # Grid spacing in x dir
    dz = dxz[1]  # Grid spacing in z dir
    vel_model_arr_P, vel_model_arr_S, vel_model_x_labels, vel_model_z_labels = (
        read_1D_vel_model_to_2D_model_for_fmm(
    ...
    vel_model_arr_P_3D = vel_model_arr_P.reshape(
        1, vel_model_arr_P.shape[0], vel_model_arr_P.shape[1]
    )
    vel_model_arr_S_3D = vel_model_arr_S.reshape(
        1, vel_model_arr_S.shape[0], vel_model_arr_S.shape[1]
    )
        ...
        for j in range(theta_grid_P.shape[1]):
            ...
            if count % 100 == 0:
                print(
                    ...
                    count,
                    "/",
                    theta_grid_P.shape[0] * theta_grid_P.shape[1],
                )
            count += 1
            ...
            node_coords = (
                np.array([0, vel_model_x_labels[i], vel_model_z_labels[j]], dtype=float)
                / 1000
            )
            node_coords = (
                node_coords + 0.001
            )  # Add 1 metre, so that node coords are non-zero
            receiver_coords = (
                np.array([0, array_centre_xz[0], array_centre_xz[1]], dtype=float)
                / 1000
            )
            ...
            ray = ray_tracer(
                vel_model_arr_P_3D / 1000,
                dxyz,
                src_loc=node_coords,
                rx_loc=receiver_coords,
            )
            # Calculate incidence angle at array:
            try:
                ray_vec_at_receiver = -np.array(
                    [ray[-1, 1] - ray[-2, 1], ray[-1, 2] - ray[-2, 2]]
                )  # Note minus sign, as defining as vector out from array
                vert_vec = np.array([0, 1])
                theta_curr = np.arccos(
                    (ray_vec_at_receiver.dot(vert_vec))
                    / (vec_norm(ray_vec_at_receiver) * vec_norm(vert_vec))
                )  # cos(theta) = a.b / |a| |b|
